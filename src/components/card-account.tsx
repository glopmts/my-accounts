...
import DownloadButton from "./download-files";
...
const AccountIcon: React.FC<AccountIconProps> = ({ icon, color, metaIcon }) => (
  <div
    className={`w-10 h-10 relative shrink-0 dark:bg-zinc-800 bg-linear-to-br ${color} rounded-lg shadow-inner flex items-center justify-center overflow-hidden`}
  >
    {icon ? (
      <Image src={icon} alt="" fill className="object-cover" />
    ) : (
      <div className="text-white dark:text-white">{metaIcon}</div>
    )}
  </div>
);

// AccountCard principal
export const AccountCard: React.FC<AccountCardProps> = ({
  ...
  const actionProps = { account, userId, refetch, onEdit, onDelete };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col mb-4 min-w-0">
        ...
        {account.url && (
          <p className="text-xs text-zinc-500 truncate mb-2">{account.url}</p>
        )}
        {account.description && (
      ...
      {/* Seletor de múltiplas senhas */}
      {hasPasswords && account.passwords.length > 1 && (
        <div className="mb-3">
